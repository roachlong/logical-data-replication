...
setup_ldr() {
    echo "Time to SETUP LDR (logical data replication) to the SECONDARY cluster"
    ...
    cockroach sql --url "$(get secondary url)" \
        -e "CREATE EXTERNAL CONNECTION 'primary' AS '${url}';"
}

start_ldr() {
    echo "Time to START LDR (logical data replication) to the SECONDARY cluster"

    db=$(get secondary database)
    url=$(get secondary url)
    if [ -z $(get secondary workload) ]; then
        echo "initializing the workload on secondary..."

        cockroach sql --url "${url}" \
            -e "CREATE DATABASE tpcc;"

        cockroach workload init tpcc ${url/${db}/tpcc}
        
        put secondary workload tpcc
    fi

    tpcc_tables="tpcc.public.customer, tpcc.public.district, tpcc.public.history, tpcc.public.item, tpcc.public.new_order, tpcc.public.order, tpcc.public.order_line, tpcc.public.stock, tpcc.public.warehouse"
    replication_sql="""
        CREATE LOGICAL REPLICATION STREAM
        FROM TABLES (${tpcc_tables})
        ON 'external://primary'
        INTO TABLES (${tpcc_tables})
        WITH MODE=validated;
    """
    
    cockroach sql --url "${url/${db}/tpcc}" \
        -e "${replication_sql}" | 
        while read -r value; do
            put secondary replication_job ${value}
        done
}

stop_ldr() {
    echo "Time to STOP LDR (logical data replication) to the SECONDARY cluster"
    
    cockroach sql --url "$(get secondary url)" \
        -e "PAUSE JOB $(get secondary replication_job);"

    while true; do
        status=$(cockroach sql --url "$(get secondary url)" \
            -e "SELECT status FROM [SHOW JOBS] WHERE job_id = $(get secondary replication_job);" \
            --format=csv | tail -n 1)
    ...
    cockroach sql --url "$(get secondary url)" \
        -e "CANCEL JOB $(get secondary replication_job);"
    ...
    cockroach sql --url "$(get secondary url)" \
        -e "DROP DATABASE tpcc;"
        
    put secondary workload ""
}
